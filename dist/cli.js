            ...
            case 1:
                ...
                switch (type) {
                    ...
                    case 'GetDeviceParameterInfo':
                        motionMasterClient.requestGetDeviceParameterInfo({ deviceAddress: deviceAddress }, messageId);
                        break;
                    case 'GetDeviceParameterValues':
                        parameters = args.map(paramToIndexAndSubindex);
                        motionMasterClient.requestGetDeviceParameterValues({ deviceAddress: deviceAddress, parameters: parameters }, messageId);
                        break;
                    case 'GetDeviceFileList':
                        motionMasterClient.requestGetDeviceFileList({ deviceAddress: deviceAddress }, messageId);
                        break;
                    case 'GetDeviceLog':
                        motionMasterClient.requestGetDeviceLog({ deviceAddress: deviceAddress }, messageId);
                        break;
                    default:
...
commander_1.default
    ...
    var deviceAddress, parameterValues, messageId;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, getCommandDeviceAddress(cmd)];
            case 1:
                deviceAddress = _a.sent();
                parameterValues = paramValues.map(paramValueToIndexAndSubindex);
                messageId = uuid_1.v4();
                exitOnMessageReceived(messageId);
                motionMasterClient.requestSetDeviceParameterValues({ deviceAddress: deviceAddress, parameterValues: parameterValues }, messageId);
                return [2 /*return*/];
        }
    });
}); });
commander_1.default
    ...
    var deviceAddress, interval, parameters, getDeviceParameterValues, messageId;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, getCommandDeviceAddress(cmd)];
            case 1:
                ...
                messageId = uuid_1.v4();
                motionMasterClient.requestStartMonitoringDeviceParameterValues({ getDeviceParameterValues: getDeviceParameterValues, interval: interval, topic: topic }, messageId);
                return [2 /*return*/];
        }
    });
}); });
...
function getCommandDeviceAddress(cmd) {
